...
def generate_launch_description():
    pkg = get_package_share_directory('rg6_description')

    default_xacro = os.path.join(pkg, 'urdf', 'rg6_instance.urdf.xacro')
    default_rviz  = os.path.join(pkg, 'rviz', 'rg6.rviz')  # optional

    # Args
    model_arg = DeclareLaunchArgument('model', default_value=default_xacro, description='RG6 instance xacro')
    ...
    prefix_arg = DeclareLaunchArgument('prefix', default_value='g1_', description='Name prefix for links/joints')
    mesh_pkg_arg = DeclareLaunchArgument('mesh_pkg', default_value='rg6_description')
    mesh_subdir_arg = DeclareLaunchArgument('mesh_subdir', default_value='meshes/visual')

    # Colors as single strings (RGBA with spaces) — keep them quoted in Command
