...
def generate_launch_description():
    pkg = get_package_share_directory('xarm5_description')

    default_xacro = os.path.join(pkg, 'urdf', 'xarm5_instance.urdf.xacro')
    default_rviz  = os.path.join(pkg, 'rviz', 'xarm5.rviz')  # optional
    ...
    prefix_arg = DeclareLaunchArgument(
        'prefix', default_value='x1_', description='Prefix for link/joint names'
    )
    mesh_pkg_arg = DeclareLaunchArgument(
        'mesh_pkg', default_value='xarm5_description',
        description='Package where meshes live'
    )
    mesh_subdir_arg = DeclareLaunchArgument(
        'mesh_subdir', default_value='meshes/visual',
        description='Relative path to visual meshes'
    )
